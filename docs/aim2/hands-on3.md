...
皆さんには Admin 権限を持ったアカウントを配布しているはずなので、それを用いてユーザーを作成してみましょう。
ユーザーに対する権限の与え方は二種類あり、直接付与する方法とグループに権限を付与し、そのグループにユーザーを所属させる方法です。
基本的にグループに権限を付与するほうが管理しやすいので、そちらを使いましょう。今回は、`only-s3-static-site-hosting`というグループがあるはずです。

IAM ユーザーを作成すると、アクセストークンが発行できるようになります。それを発行して、手元に控えておいてください。流出するとサービスをそのユーザーに付与した権限内でサービスを好き放題使われてしまうので、慎重に扱いましょう。
